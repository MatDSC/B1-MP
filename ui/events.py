    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if not (0 <= x < 10 and 0 <= y < 8):
                return True  # clic hors plateau

            clicked_piece = board.get_piece_at((x, y))

            if event.button == 1:  # clic gauche = déplacer / sélectionner
                if board.selected:
                    if (x, y) == board.selected.position:
                        board.selected = None  # déselection
            ...
            elif event.button == 3:  # clic droit = rotation
